...
void test_XOR()
{
  ...
  for (int epoch = 0; epoch < epochs; ++epoch)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      float loss = net.getLayers().back().rmse(y_pred, targets[i]);
      total_loss += loss;

      Tensor dL_dY = net.getLayers().back().dL_dY(y_pred, targets[i]);

      net.backProp(dL_dY, inputs[i]);
...
void test_LinearRegression()
{
  ...
  for (int epoch = 0; epoch < epochs; ++epoch)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      Tensor y_pred = net.forwardPass(X[i]);

      float loss = net.getLayers().back().rmse(y_pred, Y[i]);
      total_loss += loss;

      Tensor dL_dY = net.getLayers().back().dL_dY(y_pred, Y[i]);

      net.backProp(dL_dY, Y[i]);
      net.updateParameters(lr);
    ...
    if (epoch % 100 == 0)
    {
      // std::cout << "w: " << net.getLayers()[0].getWeights().print() << std::endl;
      std::cout << "epoch " << epoch
